...
def kawaii_actress_ground_extract(response: HtmlResponse):
    ...
    if len(hrefs) == 0:
       raise ExtractException("kawaii actress page ground extract error", response.url)
...
def kawaii_actress_detail_extract_id(url: str):
    ...
    if id_m is None:
        raise ExtractException('illegal kawaii actress detail page url', url)
    return id_m.group()
...
def kawaii_actress_detail_extract_name(response: HtmlResponse):
    ...
    if name is None or name_en is None:
        raise ExtractException("extract kawaii actress name error", response.url)
    return name, name_en
...
def kawaii_actress_detail_extract_profile_img(response: HtmlResponse):
    ...
    if img_url_path is None:
        raise ExtractException("extract kawaii actress profile img error", response.url)
